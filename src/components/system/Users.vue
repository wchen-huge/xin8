<template>
  <div>
      ...
      <el-table stripe :data="userList" style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作" width="180">
          <template slot-scope="scope">
            ...
            <el-button type="warning" size="small" class="el-icon-s-tools" @click="openAssignRole(scope.row.id)"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="新增用户信息" :visible.sync="addDialogVisible" width="50%" @close="closeAdd">
    ...
    <el-dialog title="分配角色" :visible.sync="roleDialogVisible" width="60%" center>
      <el-transfer style="text-align: left; display: inline-block" v-model="roleIds" :titles="['未拥有', '已拥有']"
                   :button-texts="['到左边', '到右边']" :data="roles">
      </el-transfer>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    async assignRole () {
      // 方式一
      // const { data: res } = await this.$http.put('sys/userRole/' + this.userId + '/' + this.roleIds.join(','))
      // 方式二
